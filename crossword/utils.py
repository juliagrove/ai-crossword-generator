...
def crossword(category, num_words):
    ...
    Main entry point
    """
    content = _get_crossword_content(category, num_words)  # gemini output
    crossword_filled, words_placed = _build_grid(
        content
    )  # get crossword grid and words dict
    first_letters, across_clues, down_clues = _build_clues(
        content, words_placed
    )  # get grid coordinates of first letters + word/clue pairs
    numbered_grid = _add_numbers_to_grid(
        crossword_filled, first_letters
    )  # build grid with clue numbers
    final_grid = _simplify_grid(numbered_grid)  # shrink grid removing blank space
    return final_grid, across_clues, down_clues


def _build_clues(content, words_placed):
    # add clues to words dict
    for word in content:
        if word in words_placed:
            words_placed[word]["clue"] = content[word]
...
def _build_grid(content):
    ...
    crossword_filled, words_placed = _create_crossword(words, crossword)
    ...
    for word in words:
        ...
        for letter_index, letter in enumerate(word):
            for i, row in enumerate(crossword):
                ...
                if letter in row and _can_place(
